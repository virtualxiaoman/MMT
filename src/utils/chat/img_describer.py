...
class ImageResizer:
    @staticmethod
    def resize(image: Image.Image, max_pixels: int = 768 * 768) -> Image.Image:
        """
        按总像素数等比例缩放图片。
        """
    ...
    @classmethod
    def preprocess(cls, image: Image.Image, max_pixels: int = 768 * 768) -> Image.Image:
        """
        图片预处理：
        1. 缩放
        2. RGB转换
        """
        image = cls.resize(image=image, max_pixels=max_pixels)
        image = cls.convert_to_rgb(image=image)
        return image


class ImageDescriber:
    def __init__(self, api_key: str | None = None, model: str = LLMModelType.QWEN_VL_PLUS.value,
...
2. 描述图片中的人物、动物、物体、场景、动作、表情、服装、环境等可见信息。图中的人物名称比较容易确认的时候请你给出名称（不能确认是谁就不给出）。
...
"""
        ...
        if response.status_code != 200:
            raise RuntimeError(f"图片识别失败: {response.message}")
